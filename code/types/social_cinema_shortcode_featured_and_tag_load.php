<?PHP
	...
	class social_cinema_shortcode_featured_and_tag_load{
	
		function add_ajax(){
		
			add_action( 'wp_ajax_social-cinema-load-more-tag', array($this, 'load_more') );
			add_action( 'wp_ajax_nopriv_social-cinema-load-more-tag', array($this, 'load_more') );
		...
		function display($atts){
			...
			return $videos . "</div><button post_id='" . $post->ID . "' term='" . $atts['term'] . "' source='" . $atts['source'] . "' action='social-cinema-load-more-tag' next_page_number='1' next_page='" . $video_data[1]['nextPageToken'] . "' page_total='" . $atts['total'] . "' class='social_cinema_load_more_tag'>Load More</button>";
